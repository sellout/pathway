...
canonicalizePath ::
  (FsOperations typ) => Path 'Abs typ String -> IO (Path 'Abs typ String)
canonicalizePath = either throwIO pure <=< Thin.canonicalizePath @_ @Void

-- | Don’t use this. It relies on the “current directory”.
makeAbsolute ::
  (FsOperations typ) =>
  Path ('Rel 'True) typ String -> IO (Path 'Abs typ String)
makeAbsolute = either throwIO pure <=< Thin.makeAbsolute @_ @Void

makeRelativeToCurrentDirectory ::
  (FsOperations typ) =>
  Path 'Abs typ String -> IO (Path ('Rel 'True) typ String)
makeRelativeToCurrentDirectory =
  either throwIO pure <=< Thin.makeRelativeToCurrentDirectory @_ @Void
...
getSymbolicLinkTarget ::
  (FsOperations typ) => Path 'Abs typ String -> IO (Path 'Rel.Any typ String)
getSymbolicLinkTarget =
  either throwIO pure <=< Thin.getSymbolicLinkTarget @_ @Void
