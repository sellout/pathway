...
import safe "pathway-compat-base" System.IO.Error.Pathway (FullError)
...
class Rep rep where
  ...
  --   should pass explicit absolute paths. However, for compatibility with
  --   other code that relies on the “current” directory, this is made available
  --   so that it can be scoped around such calls.
  --
  --  __NB__: Like the underlying `System.Directory.withCurrentDirectory`, this
  --          is /not/ thread-safe.
  withCurrentDirectory ::
    Path 'Abs 'Dir rep ->
    IO a ->
