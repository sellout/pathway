...
import safe "yaya" Yaya.Pattern (Maybe, XNor (Neither))
...
import safe "base" Prelude (undefined)
...
--  filename (or the absernce thereof) in the path.
...
type Reparented :: Resolution -> Kind.Type -> Kind.Type
type family Reparented res where
  Reparented 'Unres = Maybe
  Reparented 'Res = Identity

type RelPath :: Resolution -> Kind.Type -> Kind.Type
type RelPath res rep = Maybe ([Maybe rep], Reparented res rep)
...
data Path res rel typ rep = Path
  { directories :: Maybe ([Maybe rep], RelPath res rep),
...
-- | Type-level ternary function to compute TODO.
type Min :: (Resolution, Relativity) -> Resolution -> Resolution
type family Min a b where
  Min '( 'Res, 'Rel) 'Res = 'Res
  Min _ _ = 'Unres

type TotalOps :: Resolution -> Resolution -> Kind.Constraint
class TotalOps res res' where
  ...
  --          the same precedence …
  (</>) :: Path res rel 'Dir rep -> Path res' 'Rel typ rep -> Path (Min '(res, rel) res') rel typ rep

infixr 5 </>

instance TotalOps 'Res 'Res where
  parent </> child =
    Path
...
instance TotalOps 'Res 'Unres where
  parent </> child =
    Path
      { directories = directories child <> directories parent,
        filename = filename child
...
instance TotalOps 'Unres 'Res where
  parent </> child =
    Path
      { directories = directories child <> directories parent,
        filename = filename child
...
instance TotalOps 'Unres 'Unres where
  parent </> child =
    Path
      { directories = directories child <> directories parent,
        filename = filename child
